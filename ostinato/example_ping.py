...
# NEW - This program will generate 100 icmp request packets with length starting from 64 and 
# length incrementing by 1 for each packet.
...
try:
    ...
    # NEW - setup varying length for the stream
    # setup length
    s.core.len_mode = 1
    s.core.frame_len_min = 64
    s.core.frame_len_max = 1518

    # NEW - setup custom mac and ip address
    # setup stream protocols as mac:eth2:ip4:icmp:payload
    p = s.protocol.add()
    p.protocol_id.id = ost_pb.Protocol.kMacFieldNumber
    p.Extensions[mac].dst_mac = 0x080027b9e0a7
    p.Extensions[mac].src_mac = 0x080027b1048c
